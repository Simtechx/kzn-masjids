
import React from 'react';

const FindMasjidsHero: React.FC = () => {
  return (
    <section className="py-16 px-4 bg-islamic-dark-green text-white">
      <div className="container mx-auto text-center">
        <h2 className="text-4xl md:text-5xl font-bold mb-4">Find Masjids & Musallahs in KwaZulu-Natal</h2>
        <p className="text-xl max-w-4xl mx-auto">
          Discover prayer spaces across KZN's 5 regions - Northern Natal, South Coast, Durban,
          Midlands, North Coast.
        </p>
      </div>
    </section>
  );
};

export default FindMasjidsHero;
