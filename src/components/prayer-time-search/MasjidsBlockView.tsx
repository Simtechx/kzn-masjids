...
const MasjidsBlockView: React.FC<MasjidsBlockViewProps> = ({
  ...
  const prayerColors = {
    fajr: 'bg-pink-100 border-pink-400',
    dhuhr: 'bg-amber-100 border-amber-500',
    asr: 'bg-emerald-100 border-emerald-500',
    isha: 'bg-indigo-100 border-indigo-500',
  };
  ...
  const selectedColors = {
    fajr: 'bg-pink-400 border-pink-600',
    dhuhr: 'bg-amber-500 border-amber-700',
    asr: 'bg-emerald-500 border-emerald-700',
    isha: 'bg-indigo-500 border-indigo-700',
  };

  // Define darker colors for badges (EARLIEST/LATEST when not selected)
  const badgeColors = {
    fajr: 'bg-pink-400',
    dhuhr: 'bg-amber-500',
    asr: 'bg-emerald-500',
          ...
          <div className="grid grid-cols-2 gap-2">
            {prayerTypes.map((prayer) => {
              ...
              return (
                <div 
                  key={prayer}
                  className={`p-3 rounded text-center border-2 min-h-[80px] flex flex-col justify-center ${
                    isSelected
                  ...
                  {extremeType && (
                    <Badge className={`mt-1 text-[10px] px-2 py-0 mx-auto ${
                      isSelected 
                        ...
                        : `text-white ${badgeColors[prayer]} border-0`
                    }`}>
